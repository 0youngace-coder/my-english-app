...
# Word 2007(.docx) 생성 함수 - GitHub/Cloud용 (메모리에 생성)
...
if os.path.exists(SAVE_DIR):
    ...
    if saved_files:
        ...
        if selected_file:
            file_path_full = os.path.join(SAVE_DIR, selected_file)
            if selected_file.endswith('.docx'):
                st.sidebar.info(f"{selected_file}은 Word 파일입니다. 폴더에서 더블클릭하세요.")
            else:
